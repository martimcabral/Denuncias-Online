...
$sql = "SELECT * 
        FROM `sugestoes_rgpc` 
        WHERE `numero_ticket` = (SELECT MAX(`numero_ticket`) FROM `sugestoes_rgpc`)";
...
$sql = "INSERT INTO sugestoes_rgpc SET
    numero_ticket = ?,
	descricao = ?,
	utilizador = ?,
	ip = ?,
...
if ($stmt->execute()) {
	echo '<script>alert("Sugestão inserida com sucesso!")</script>';
